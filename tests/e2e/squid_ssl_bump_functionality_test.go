...
var _ = Describe("Squid SSL-Bump Functionality", Ordered, func() {
	...
	const testServerURL = "https://test-server.caching.svc.cluster.local:443"
	...
	BeforeEach(func() {
		...
		cachingCAConfigMap, err := k8sClient.CoreV1().ConfigMaps(namespace).Get(context.Background(), "caching-ca-bundle", metav1.GetOptions{})
		Expect(err).NotTo(HaveOccurred(), "Failed to get caching-ca-bundle ConfigMap")
	...
	Describe("SSL-Bump Log Verification", func() {
		It("should show decrypted HTTPS requests in Squid access logs", func() {
			...
			Expect(logOutput).To(ContainSubstring("test-server.caching.svc.cluster.local"), "Should show the test server URL in logs")
